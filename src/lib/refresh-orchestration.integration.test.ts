...
testDescribe("refresh orchestration integration", () => {
  ...
  it("queues a single refresh job and dedupes the second enqueue", async () => {
    ...
    const [{ queueSingleFeedRefresh }, { refreshQueueName }] = await Promise.all([
    ...
    redis = new IORedis(testRedisUrl, { maxRetriesPerRequest: null, lazyConnect: true });
    ...
    try {
      const first = await queueSingleFeedRefresh(user.id, feed.id, "manual", {
      ...
      const second = await queueSingleFeedRefresh(user.id, feed.id, "manual", {
        createRefreshJob: prisma.refreshJob.create.bind(prisma.refreshJob),
        deleteRefreshJob: prisma.refreshJob.delete.bind(prisma.refreshJob),
        enqueueRefresh: async (payload) => {
          const helper = await import("@/lib/queue");
      ...
      const refreshJobs = await prisma.refreshJob.findMany({ where: { feedId: feed.id } });
      ...
      await prisma.item.deleteMany({ where: { feedId: feed.id } }).catch(() => null);
      await prisma.refreshJob.deleteMany({ where: { feedId: feed.id } }).catch(() => null);

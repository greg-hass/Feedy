...
export function shouldFetchReadableContent(
  ...
  return Boolean(options?.allowRedditExternalArticles && isExternalRedditArticle(input));
}
...
export function shouldRenderReaderLeadMedia(input: ReaderContentInput & {
  ...
  if (input.feed.sourceType === "REDDIT_RSS" && readerHtml && /<img\b/i.test(readerHtml)) {
    return false;
  }

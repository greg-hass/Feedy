...
const envSchema = z.object({
	DATABASE_URL: z
		...
		.min(1)
		.default(
			"postgresql://postgres:postgres@localhost:5432/feedy?schema=public",
		),
	...
	REFRESH_DEFAULT_INTERVAL_MINUTES: z.coerce
		.number()
		.int()
		.min(5)
		.max(1440)
		.default(15),
	REFRESH_WORKER_CONCURRENCY: z.coerce
		.number()
		.int()
		.min(1)
		.max(100)
	...
	READER_EXTRACTION_WORKER_CONCURRENCY: z.coerce
		.number()
		.int()
		.min(1)
		.max(8)
	...
	REFRESH_HTTP_TIMEOUT_MS: z.coerce
		.number()
		.int()
		.min(2000)
		.max(60_000)
...
const parsedEnv = envSchema.parse({
	...
	REFRESH_DEFAULT_INTERVAL_MINUTES:
		process.env.REFRESH_DEFAULT_INTERVAL_MINUTES,
	...
	READER_EXTRACTION_WORKER_CONCURRENCY:
		process.env.READER_EXTRACTION_WORKER_CONCURRENCY,
...
export function getProductionEnvProblems(
	config: ProductionEnvConfig & { REDDIT_PROXY_URL?: string },
) {
	...
	if (
		...
		problems.push(
			"AUTH_SECRET must be a random value of at least 32 characters",
		);
	}

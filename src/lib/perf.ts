...
export function logPerf(name: string, durationMs: number, meta?: PerfMeta, force = false) {
  if (!env.PERF_LOGGING && !force) {
    return;
  }
  ...
  console.info(`[perf] ${name} durationMs=${Math.round(durationMs)}${serializeMeta(meta)}`);
}

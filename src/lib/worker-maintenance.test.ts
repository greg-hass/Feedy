...
import type { RefreshJobPayload } from "@/lib/queue";
	...
	it("requeues stale running jobs on startup", async () => {
		...
		const client = {
			refreshJob: {
				findMany: async (args: {
					where: { status: string };
					select: { id: true; feedId: true; trigger: true };
				}) => {
				...
				updateMany: async (args: {
					...
					calls.push(
						args.where.id
							? `update:${args.where.id.in.join(",")}`
							: `update:${args.where.status}`,
					);
		...
		const count = await recoverStaleRefreshJobs(
			client as never,
			(async (payload: RefreshJobPayload) => {
				calls.push(
					`enqueue:${payload.refreshJobId}:${payload.feedId}:${payload.trigger}`,
				);

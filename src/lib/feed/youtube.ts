...
export function probeYouTubeShort(videoId: string): Promise<boolean | null> {
	...
	const probe = (async () => {
		try {
			...
			const resolvedUrl =
				(response as Response & { finalUrl?: string }).finalUrl ?? response.url;
...
function firstThumbnailUrlFromParsedItem(
	value: ParsedYouTubeItem["mediaThumbnail"],
) {
...
async function mapFeedItemToItem(
	feedId: string,
	item: ParsedYouTubeItem,
): Promise<ParsedFeedItem> {
	const videoId =
		(typeof item.ytVideoId === "string" ? item.ytVideoId : null) ??
		(typeof item.id === "string" && item.id.startsWith("yt:video:")
...
export async function validateYouTubeFeedUrl(
	url: string,
	...
	return {
		...
		description: decodeHtmlEntities(
			readText((feed as { description?: unknown }).description) || null,
		),
...
export async function fetchYouTubeFeedConditionally(
	...
	const items = await Promise.all(
		(feed.items ?? []).map((item) =>
			mapFeedItemToItem(feedId, item as ParsedYouTubeItem),
		),
	);

...
describe("youtube feed helpers", () => {
	...
	it("ignores non-feed youtube urls", () => {
		assert.equal(
			parseYouTubeFeedTarget("https://www.youtube.com/@GoogleDevelopers"),
			null,
		);
	});
	...
	it("rejects lookalike youtube hosts", () => {
		assert.equal(
			parseYouTubeFeedTarget(
				"https://youtube.com.attacker.invalid/feeds/videos.xml?channel_id=proof",
			),
			null,
		);
	});

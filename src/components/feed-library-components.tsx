...
export function FeedRow({ feed, feeds, index }: { feed: NavFeed; feeds: NavFeed[]; index: number }) {
        ...
        >
          <Link href={`/app/feeds/${feed.id}`} className="flex min-w-0 flex-1 items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <FeedMetadataRow feed={feed} onShowHealth={() => setShowHealth(true)} health={health} />
            </div>
          </Link>
        </div>
      </SwipeRow>
...
export function FolderRow({ folder, folders, index }: { folder: NavFolder; folders: NavFolder[]; index: number }) {
  const [showEdit, setShowEdit] = useState(false);
  const queryClient = useQueryClient();

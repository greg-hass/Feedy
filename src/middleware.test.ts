...
describe("Security headers", () => {
	...
	it("does not allow arbitrary inline scripts", () => {
		...
		assert.ok(
			...
			"script-src should not allow arbitrary inline scripts",
		);
	});
	...
	it("allows YouTube iframe API and embeds in CSP", () => {
		...
		assert.ok(
			csp?.includes("script-src 'self'") &&
				csp.includes("https://www.youtube.com"),
			"CSP should allow YouTube iframe API script",
		);
		assert.ok(
			csp?.includes("frame-src https://www.youtube.com"),
			"CSP should allow YouTube embeds",
		);
	});
	...
	it("allows https images for feed thumbnails", () => {
		...
		assert.ok(
			...
			"CSP should allow external images",
		);
	});
